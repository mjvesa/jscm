...
(.interactionEnvironment$ (Scheme.currentEvaluator) (com.github.mjvesa.jscm.jsint.DynamicEnvironment.))


;; this is a convenience definition so we can write Op.add instead of jsint.Op.add, etc.

(set! import (lambda(S) (begin (com.github.mjvesa.jscm.jsint.Import.addImport (.toString S)) #t)))
(import "com.github.mjvesa.jscm.jscheme.*")
...
(set! list (lambda R R))
(set! pair? (lambda(x) (.isInstance Pair.class x)))
(set! cons Pair.)
(set! first .first$)
(set! rest .rest$)
...
(define (throw x)
   (Procedure.throwRuntimeException
      (JschemeThrowable. x)))
...
(define-macro (let bindings . body)
    (define (varval v) (Symbol.intern (.concat (.toString v) "=")))
...
(define-macro (let* bindings . body )
     (if (null? bindings) (Scheme.toBody body)
     ...
     (let* ((specifier (first R))
            ...
            (symarray 
             (if (or (equal? symbols #null) (equal? symbols 'all))
                  #null
                 (list->array Symbol.class symbols))))
     (case specifier
       ((import-procedures)
